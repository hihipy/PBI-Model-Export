...
The output file name is generated automatically from the report name — no manual naming needed. If you have `FTE Funding Dashboard.pbix` open, the file saves as `FTE Funding Dashboard_ModelExport.json`.
...
"dataHead": {
  "FYs": {
    "sourceFile": "FY26.xlsx",
    "columns": ["Cost Ctr Level 6 Name - Default Org", "Empl Type Prim Pos", "Fiscal Year (Acct)", "Empl ID", "Empl Name", "Driver", "Driver Type", "Total", "MSOM Classification", "CARTO Classification", "Department", "FTE"],
    "rows": [
      ["Level 06 - MSOM - Faculty And Professional Affairs", "Faculty-UMMG", "FY2026", "50017271", "Joan Elizabeth St. Onge", "PG006192", "Program", "2791.42", "Graduate Tuition Funded Teaching Support", "Teaching", "Faculty and Professional Affairs", "0.043"],
      ["Level 06 - MSOM - Faculty And Professional Affairs", "Faculty-UMMG", "FY2026", "50017271", "Joan Elizabeth St. Onge", "PG003542", "Program", "25635.58", "UMMG", "Clinical", "Faculty and Professional Affairs", "0.395"]
    ]
  }
}
```

> **Note on numeric precision:** XLSX stores floating point values at full IEEE 754 precision. FTE values may appear as scientific notation strings (e.g. `4.3E-2`). Parse as `double` and round downstream as needed.

## Output Format

```json
{
  "exportDate": "2026-04-13 13:46:21",
  "dashboardName": "FTE Funding Dashboard",
  "modelName": "0117e485-4246-4d03-bfce-234722c63dff",
  "modelMetadata": {
  ...
  "dataSources": [ ... ],
  "summary": {
    "userFacingTables": ["FYs"],
    "autoGeneratedTables": [],
    "topLevelKPIs": ["Research $", "Research FTE", "Clinical $", "..."],
    "measuresByFolder": {
      "00 - KPIs": ["Total $", "Total FTE", "Active Appointments"],
      "03 - Research": ["Research $", "Research FTE", "% Research No CS"]
    },
    "relationshipMap": [],
    "columnInventory": {
      "FYs": ["Empl ID (Int64)", "Empl Name (String)", "Driver Type (String)", "FTE (Double)", "..."]
    },
    "measuresPerTable": {
      "FYs": 24
    }
...
- "What does the actual data look like — what are the distinct values in CARTO Classification?"
...
with open("FTE_Funding_Dashboard_ModelExport.json", "r", encoding="utf-8") as f:
    model = json.load(f)
...
```powerquery
let
    Source = Json.Document(File.Contents("FTE_Funding_Dashboard_ModelExport.json")),
